...
export type {
  ...
  TaskResolved,
  TaskResult,
} from "./types"; //Overly-aggressive byte-shaving
...
const createTakePattern = <S>(
  ...
  const take = async <P extends AnyListenerPredicate<S>>(
    ...
    const promises: (Promise<null> | Promise<[Action, S, S]>)[] = [
      tuplePromise,
    ];
...
>(middlewareOptions: CreateListenerMiddlewareOptions<ExtraArgument> = {}) {
  ...
  const middleware: ListenerMiddleware<S, D, ExtraArgument> = (api) => (
    next
  ) => (action) => {
    if (!isAction(action)) {
