import type {
  AsyncDataStoreActions,
  AsyncReturn,
  DataStoreActions,
  IDataStore,
} from "@bungie/datastore/types";
...
export abstract class DataStore<
    ...
    TObserverType extends BroadcasterObserver<
      TDataType,
      TObserverProps
    > = BroadcasterObserver<TDataType, TObserverProps>
  >
  extends Broadcaster<TDataType, TObserverProps, TObserverType>
  implements IDataStore<TDataType, TObserverProps, any> {
    ...
    return Object.keys(actions).reduce((acc, item: keyof T) => {
      ...
      const func = (...args: any[]) => {
        ...
        if (!result || typeof result !== "object") {
          throw new Error(
            "Actions must output data that is compatible with the DataStore's state. Received null, undefined, or a non-object result."
          );
        }

        // Determine whether the result is a promise
        const isAsync =
          result[Symbol.toStringTag] === "AsyncFunction" ||
  ...
  private nextTick(callback: () => void) {
    ...
    Promise.allSettled(pendingUpdatePromises).finally(callback);
  }
 ...
 */
export function getDataStoreBuilder<
  TDataType extends Record<string, any>,
  TObserverProps = any,
  TObserverType extends BroadcasterObserver<
    TDataType,
...
>() {
  ...
  const build = <T extends DataStoreActions<TDataType>>(buildParams: {
    ...
    dataStoreParams?: BroadcasterParams<
      TObserverType,
      TDataType,
      TObserverProps
    >;
    ...
    })(buildParams.initialState, buildParams.dataStoreParams) as IDataStore<
      TDataType,
      TObserverProps,
      T
    >;
