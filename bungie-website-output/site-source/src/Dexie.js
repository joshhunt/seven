...
export default function Dexie(dbName, options) {
  ...
  function init() {
    ...
    db.on("versionchange", function (ev) {
      ...
      if (ev.newVersion > 0)
        console.warn(
          `Another connection wants to upgrade database '${db.name}'. Closing db now to resume the upgrade.`
        );
      else
        console.warn(
          `Another connection wants to delete database '${db.name}'. Closing db now to resume the delete request.`
        );
      db.close();
    ...
    db.on("blocked", (ev) => {
      if (!ev.newVersion || ev.newVersion < ev.oldVersion)
        ...
        console.warn(
          `Upgrade '${db.name}' blocked by other connection holding version ${
            ev.oldVersion / 10
          }`
        );
    });
  ...
  function runUpgraders(oldVersion, idbtrans, reject) {
    ...
    newScope(function () {
      ...
      if (oldVersion === 0) {
        ...
        Promise.follow(() => db.on.populate.fire(trans)).catch(
          rejectTransaction
        );
  ...
  function updateTablesAndIndexes(oldVersion, trans, idbtrans) {
    ...
    var queue = [];
    var oldVersionStruct = versions.filter(
  ...
  this.open = function () {
    if (isBeingOpened || idbdb)
      return dbReadyPromise.then(() =>
        dbOpenError ? rejection(dbOpenError) : db
      );
        ...
        req.onupgradeneeded = wrap(function (e) {
          ...
          if (autoSchema && !db._allowEmptyDB) {
            ...
            delreq.onsuccess = delreq.onerror = wrap(function () {
              reject(
                new exceptions.NoSuchDatabase(`Database ${dbName} doesnt exist`)
              );
            });
          } else {
  ...
  function BulkErrorHandlerCatchAll(errorList, done, supportHooks) {
    return (supportHooks ? hookedEventRejectHandler : eventRejectHandler)(
      (e) => {
        errorList.push(e);
        done && done();
      }
  ...
  props(Table.prototype, {
    ...
    _trans: function getTransaction(mode, fn, writeLocked) {
      ...
      return trans && trans.db === db
        ? trans === PSD.trans
          ...
          : newScope(() => trans._promise(mode, fn, writeLocked), {
              trans: trans,
    ...
    orderBy: function (index) {
      return new Collection(
        new WhereClause(this, isArray(index) ? `[${index.join("+")}]` : index)
      );
    },
        ...
        (resolve, reject, idbstore) => {
          ...
          const done = (result) => {
            if (errorList.length === 0) resolve(result);
              ...
              reject(
                new BulkError(
                  `${this.name}.bulkPut(): ${errorList.length} of ${numObjs} operations failed`,
                  errorList
                )
              );
          };
          var req,
            ...
            table = this;
          if (
            this.hook.creating.fire === nop &&
            this.hook.updating.fire === nop
          ) {
            //
            ...
            var effectiveKeys =
              keys ||
              (idbstore.keyPath &&
                objects.map((o) => getByKeyPath(o, idbstore.keyPath)));
            // Generate map of {[key]: object}
            var objectLookup =
              effectiveKeys &&
              arrayToObject(
                effectiveKeys,
                (key, i) => key != null && [key, objects[i]]
              );
            var promise = !effectiveKeys
              ...
              : // Keys provided. Either as inbound in provided objects, or as a keys argument.
                ...
                table
                  .where(":id")
                  .anyOf(effectiveKeys.filter((key) => key != null))
                  .modify(function () {
      ...
      return this._idbstore(READWRITE, function (
        ...
        function done(result) {
          if (errorList.length === 0) resolve(result);
            ...
            reject(
              new BulkError(
                `${self.name}.bulkAdd(): ${errorList.length} of ${numObjs} operations failed`,
                errorList
              )
            );
        }
        ...
        return this._trans(
          ...
          () =>
            this.where(":id")
              ...
              .then((count) =>
                count === 0 ? this.add(obj, key) : effectiveKey
              ),
          "locked"
  ...
  function readGlobalSchema() {
    ...
    dbStoreNames.forEach(function (storeName) {
      var store = trans.objectStore(storeName),
        ...
        dotted =
          keyPath && typeof keyPath === "string" && keyPath.indexOf(".") !== -1;
...
(() => {
  ...
  try {
    // https://github.com/dfahlander/Dexie.js/issues/617
    if (typeof localStorage !== undefined && _global.document !== undefined) {
      // Have localStorage and is not executing in a worker. Lets migrate from Dexie 1.x.
      JSON.parse(localStorage.getItem(DBNAMES) || "[]").forEach((name) =>
        dbNamesDB.dbnames.put({ name: name }).catch(nop)
      );
