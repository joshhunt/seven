...
import { StatsDefinitions } from "@Areas/GameHistory/Pgcr/StatsDefinitions";
...
export class PgcrUtils {
  ...
  public static createStats = (
    ...
    newStats.statBarData = activityData.isGambit
      ? StatsDefinitions.compPveTeamStatBarStats.slice(0, 5)
      : StatsDefinitions.teamStatBarStats.slice(0, 4);
    ...
    if (activityData.isScoredPvE) {
      ...
      newStats.leaderStatItemData = StatsDefinitions.compPveTeamStatBarStats
        .filter((stat) => stat !== "teamScore")
        .slice(0, 4);
    } else {
      newStats.leaderStatItemData = StatsDefinitions.teamStatBarStats
        .filter((stat) => stat !== "teamScore" && stat !== "score")
        .slice(0, 4);
    }
